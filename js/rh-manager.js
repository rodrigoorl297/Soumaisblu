  ...
  async function _syncRhUserFromEmployee(emp, password, opts = {}) {
    ...
    /* Cadastro no RH da empresa (não painel parceiro): tira da rede parceira e reativa login.
      ...
      Só roda quando ESTA gravação tem empresa válida vinculada (temEmpresaValida) —
      senão a pessoa acabou de virar parceiro e não pode perder o partner_root_id. */
    const inPartnerPanel = typeof window !== 'undefined' && !!window.PARTNER_ROOT_ID;
    const temEmpresaValida = opts.temEmpresaValida === true;
    if (!inPartnerPanel && existingRole !== 'parceiro' && temEmpresaValida) {
  ...
  function onEmpRoleChange() {
    ...
    const empresasOk = typeof _rhAllowedCompanies === 'function'
      ? _rhAllowedCompanies(_allCompanies || [])
      : (_allCompanies || []);
    const cnpjsOk = new Set(
      (empresasOk || []).map((e) => _digits(e.cnpj || e.cnpj_registro || ''))
    );
    const temEmpresaValida = !!(cnpjReg && cnpjsOk.has(cnpjReg));
    // --- FIM NOVO ---
    ...
    // --- NOVO: regra de negócio — sem empresa válida vinculada, vira parceiro ---
    // Não mexe em perfis protegidos (master/financeiro/etc.) já resolvidos acima.
    if (!temEmpresaValida && !_RH_PROTECTED_USER_ROLES.includes(finalRole)) {
      if (finalRole !== 'parceiro') {
        showToast('Sem empresa cadastrada válida. Este cadastro será salvo como Parceiro.', 'warning', 8000);
      }
      finalRole = 'parceiro';
    }
    // --- FIM NOVO ---
    ...
    try {
      ...
      try {
        ...
        if (password) {
          row.user_id = await _syncRhUserFromEmployee(row, password, { temEmpresaValida });
        } else if (email || prev.user_id || cpf) {
          row.user_id = await _syncRhUserFromEmployee(row, null, { syncRole: roleChanged, temEmpresaValida });
        }
        ...
        if (needsLogin && (password || limiteClubeEarly > 0)) {
          ...
          row.user_id = await _syncRhUserFromEmployee(row, syncPwd, { temEmpresaValida });
